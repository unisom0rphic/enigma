...
*   **Гибридный AI:** Сочетание NLP для обработки конфиденциальных данных и облачной модели для качественной генерации ответов.
...
2.  **Локальная обработка (Privacy-first):** Письмо обрабатывается NLP. Извлекаются PII-данные (ФИО, телефон), исправляются опечатки и формируется краткая суть проблемы. Данные остаются внутри контура сервера.
3.  **Поиск решения (RAG):** Суть проблемы векторизуется моделью **E5-base** (поддерживает мультиязычные тексты), в локальной базе **FAISS** ищутся релевантные инструкции из PDF-документации.
...
**AI & ML:**
*   **NLP/Parsing:** pymorphy3 + natasha, regex
...
### Обработка писем
NLP-модуль работает в синхронном режиме внутри эндпоинта `/process`. Система спроектирована так, чтобы не блокировать основной поток FastAPI, используя `run_in_executor` для тяжелых вычислений на CPU.
...
2.  Отправьте POST запрос на эндпоинт `/api/reload-kb`.
